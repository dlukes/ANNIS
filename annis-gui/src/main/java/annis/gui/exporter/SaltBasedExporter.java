...
/**
 * An abstract base class for exporters that use Salt subgraphs to produce some
 * kind of textual output.
 * 
...
public abstract class SaltBasedExporter implements ExporterPlugin, Serializable {
	...
	@Override
	public Exception convertText(String queryAnnisQL, int contextLeft, int contextRight, Set<String> corpora,
			List<String> keys, String argsAsString, boolean alignmc, WebResource annisResource, Writer out,
			EventBus eventBus, Map<String, CorpusConfig> corpusConfigs) {
		...
		try {
			...
			List<QueryNode> nodes = resource.path("query/parse/nodes")
					.queryParam("q", Helper.encodeJersey(queryAnnisQL)).get(new GenericType<List<QueryNode>>() {
					});
	...
	private void convertSaltProject(SaltProject p, List<String> annoKeys, Map<String, String> args, boolean alignmc,
			int offset, Map<String, CorpusConfig> corpusConfigs, Writer out, Integer nodeCount)
	...
	public abstract void outputText(SDocumentGraph graph, boolean alignmc, int recordNumber, Writer out)
			throws IOException, IllegalArgumentException;
